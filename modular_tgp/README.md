# The modularization handbook - TGP style, v0.2
...
And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `modular_tgp/modules/shuttle_toggle`
...
We will no longer have \_tgp map versions.
...
- **_Example:_** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `modular_tgp/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'modular_tgp/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'modular_tgp/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `modular_tgp/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`modular_tgp/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `modular_tgp/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes TGP edits are preferable. Just try to use your common sense about it.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. TGP edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  //TGP EDIT ADDITION BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
...
- **Removal:**
  ...
  ```byond
  //TGP EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  /*
  ...
  */
  ...
  ```byond
  //TGP EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Moved to modular_tgp/shuttle_toggle/randomverbs.dm)
  /*
...
- **Change:**
  ...
  ```byond
  //TGP EDIT CHANGE BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  //if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE) - TGP EDIT - ORIGINAL
...
That folder is **`code/__DEFINES/~tgp_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /modular_tgp/modules/example_module/code/disease_mob.dm
- Bad: /modular_tgp/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /modular_tgp/modules/example_module/icons/mining_righthand.dmi
- Bad: /modular_tgp/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for TGP UIs.
...
You can do both `// TGP EDIT` and `/* TGP EDIT */`, though in some cases you may have to use one over the other.
...
<Button
	onClick={() => act('spin', { high_quality: true })}
	// TGP EDIT ADDITION START - another example, multiline changes
	icon="rat"
...
```js
// THIS IS A TGP UI FILE
```

This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a TGP edit comment in a modular TGUI file.
