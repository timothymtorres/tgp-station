 ...
 * * persistence_save - Bool that is used when a map is loaded via persistence which affects station traits - TGP EDIT ADDITION
 *
 * Returns the config for the map to load.
 */
/proc/load_map_config(filename = null, directory = null, error_if_missing = TRUE, persistence_save = FALSE) // TGP EDIT CHANGE - /proc/load_map_config(filename = null, directory = null, error_if_missing = TRUE)
	...
	if(filename) // If none is specified, then go to look for next_map.json, for map rotation purposes.
		...
		if(directory)
			if(!(directory in MAP_DIRECTORY_WHITELIST) && !CONFIG_GET(flag/persistent_save_enabled)) // TGP EDIT CHANGE - if(!(directory in MAP_DIRECTORY_WHITELIST))
	...
	if (!configuring_map.LoadConfig(filename, error_if_missing, persistence_save)) // TGP EDIT CHANGE - if (!configuring_map.LoadConfig(filename, error_if_missing))
...
/datum/map_config/proc/LoadConfig(filename, error_if_missing, persistence_save) // TGP EDIT CHANGE - /datum/map_config/proc/LoadConfig(filename, error_if_missing)
	if(!fexists(filename))
		if(error_if_missing)
