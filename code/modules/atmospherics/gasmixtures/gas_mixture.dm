...
/// Rounds all temperature and gases to 0.1 and skips any gases less than that amount - TGP EDIT
/datum/gas_mixture/proc/to_string()
	var/list/cached_gases = gases
	var/rounded_temp = round(temperature, 0.1) // TGP EDIT

	var/list/atmos_contents = list()
	var/temperature_str = "TEMP=[num2text(rounded_temp)]"

	if(!length(cached_gases) || total_moles() < 0.1) // TGP EDIT
	...
	for(var/gas_path in cached_gases)
		...
		gas_moles = round(gas_moles, 0.1) // TGP EDIT
		if(gas_moles >= 0.1) // TGP EDIT
